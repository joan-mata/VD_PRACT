...
def main():
    ...
    # 4. Construir listas de categorías, rangos y comités
    categories = ['CADETE', 'AMATEUR', 'JUVENIL']
    cat_to_idx = {c: idx for idx, c in enumerate(categories)}

    # Grupos de rango FCA para la visualización de Acto 3
    # Jerarquía: 3a Fed/Nacional > 1a Cat/Élite > 2a Cat > 3a Cat/Sit.Especial > Cursetista
    rango_grupos = [
        'Cursetista',
        '3a Cat / Sit. Especial',
        '2a Catalana',
    ...
    def fca_to_rango_idx(categoria):
        if not isinstance(categoria, str) or not categoria.strip():
            return 0  # Sin categoría → Cursetista/Formació
        c = categoria.strip()
        # Nivel más alto: 3a Federació, RFEF, División
        if any(x in c for x in ['3a Federació', '3a Fed', 'RFEF', 'Divisió', 'Nacional', 'Excelència']):
            return 4
        # 1a Catalana + Lliga Elit (unificados)
        if any(x in c for x in ['1a Catalana', 'Lliga Elit', 'Elit']):
            return 3
        # Assistents de nivells alts equivalen a la categoria activa un nivell per sota
        if 'Assistent' in c and any(x in c for x in ['1a Catalana', 'Lliga Elit', 'Elit', '3a Federació']):
            return 3
        # 2a Catalana
        if '2a Catalana' in c:
            return 2
        if 'Assistent' in c and '2a Catalana' in c:
            return 2
        # 3a Catalana i Situació Especial (mateix nivell)
        if '3a Catalana' in c or 'Sit.Especial' in c or 'Sit. Especial' in c or 'Especial' in c:
            return 1
        # Cursetistes i formació
        return 0
    ...
    # Pre-computar la categoría de partido más habitual por árbitro (CADETE/JUVENIL/AMATEUR)
    # La columna 'Categoria' de Arbitros.csv contiene el rango FCA ("Àrbitre 3a Catalana", etc.)
    # que no coincide con las categorías de partido; usamos la categoría que más arbitran.
    ref_primary_cat = (
        df_part_terr.groupby('arbitro_principal_clean')['categoria']
        .agg(lambda x: x.value_counts().index[0] if len(x) > 0 else None)
        .to_dict()
    )

    for idx, row in df_arb_active.iterrows():
        cat = ref_primary_cat.get(row['Nom_clean'])
        ...
        ref_cat_idx = cat_to_idx.get(cat, -1) if cat else -1
        ref_comite_idx = comite_to_idx.get(comite_str, -1)
        ref_rango_idx = fca_to_rango_idx(row['Categoria'])

        referees_list.append({
            'id': f"COL_{idx}",
            'cat_idx': ref_cat_idx,
            'comite_idx': ref_comite_idx,
            'rango_idx': ref_rango_idx,
            'vet': ref_vet,
            'idp': ref_idp,
            'partidos': ref_part
        })
    ...
    with open(output_js_path, 'w', encoding='utf-8') as f:
        ...
        f.write("export const RANGOS = ")
        f.write(json.dumps(rango_grupos, ensure_ascii=False))
        f.write(";\n\n")
